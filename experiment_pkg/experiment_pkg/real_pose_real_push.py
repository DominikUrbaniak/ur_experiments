...
n_speed_values = 1000
config_filename = 'src/control_pkg/config/real_pose_real_push.ini'
...
output_dir = "docs/data/logging/"+folder+"/"
...
class RealPoseRealPush(Node):
    ...
    def pose_callback(self, msg):
        ...
        #        self.get_logger().warn(f'Timeout triggered at {experiment_timeout}s!')
            ...
            if self.timeout or self.terminated:
                ...
                if perform_logging and not self.terminated:
                    ...
                    with open(output_log_filename, 'w') as log_file:
                        log_file.write('Counter;Execution Time;Distance to Goal;Time Stamp Image Published;'
            ...
            #if not self.reset:
            log_message = (
                f"{self.counter};{execution_time:.4f};{self.distance_to_goal:.4f};{msg.stamp_ns_image_published};{msg.stamp_ns_image_received};{msg.stamp_ns_pose_published};{stamp_pose_received};{self.time_stamp_control};{time_stamp_eef};"
                f"{self.action[0]:.4f};{self.action[1]:.4f};{self.action[2]:.4f};{self.action[5]:.4f};{self.tag_found};{self.cube_pose.position.x:.4f};{self.cube_pose.position.y:.4f};{self.cube_pose.position.z:.4f};{self.cube_yaw:.4f};{eef_x:.4f};{eef_y:.4f};{eef_z:.4f};{eef_yaw:.4f};{timeout_triggered};{time_since_last_req_ms};{msg_size_image};{msg_size_poses}"
            )
