...
#        self.counter = self.counter + 1
    ...
    def timer_callback(self):
        ...
        z = 0
        if(len(tag_poses) == 1):
            x = tag_poses[0].pose.pose.position.x
            y = tag_poses[0].pose.pose.position.y
            z = tag_poses[0].pose.pose.position.z
        ...
        if req_time-self.init_time < self.communication_duration:
            ...
            if self.logging_active:
                log_message = (
                    f"{req_time_ns};{res_time_ns};{(res_time_ns-req_time_ns)/1000/1000};{len(tag_poses)};{x};{y};{z};{msg_size_req};{msg_size_res}"
                )
